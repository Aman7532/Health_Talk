...
def create_index_pattern():
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            # If we can't create the pattern through the API, let's provide instructions for manual creation
            print("\nPlease create the index pattern manually in Kibana:")
            print("1. Open Kibana in your browser at http://localhost:30002")
            print("2. Go to Management > Stack Management > Kibana > Index Patterns")
            print("3. Click 'Create index pattern'")
            print("4. Enter 'healthcare-chatbot-logs-*' as the index pattern name")
            print("5. Select '@timestamp' as the Time field")
            print("6. Click 'Create index pattern'")
            
            return "healthcare-chatbot-logs"
    except Exception as e:
        print(f"Error creating index pattern: {e}")
        
        # Provide manual instructions in case of error
        print("\nPlease create the index pattern manually in Kibana:")
        print("1. Open Kibana in your browser at http://localhost:30002")
        print("2. Go to Management > Stack Management > Kibana > Index Patterns")
        print("3. Click 'Create index pattern'")
...
def create_visualization(index_pattern_id):
    ...
    # Create visualization for log levels
    log_level_viz = {
        "attributes": {
    ...
    visualizations.append(("healthcare-chatbot-log-levels", log_level_viz))
    
    # Create visualization for log messages over time
    log_timeline_viz = {
        "attributes": {
...
visualizations.append(("healthcare-chatbot-log-timeline", log_timeline_viz))

# Create visualization for top log messages
top_messages_viz = {
    "attributes": {
    ...
    visualizations.append(("healthcare-chatbot-top-messages", top_messages_viz))
...
def create_dashboard(index_pattern_id, viz_ids):
    ...
    for i, viz_id in enumerate(viz_ids):
        if viz_id == "healthcare-chatbot-log-levels":
            # Log levels pie chart in top left
            panels.append({
                "id": viz_id,
                "type": "visualization",
                "panelIndex": str(i+1),
        ...
        elif viz_id == "healthcare-chatbot-log-timeline":
            # Log timeline chart in middle
            panels.append({
                "id": viz_id,
                "type": "visualization",
                "panelIndex": str(i+1),
        ...
        elif viz_id == "healthcare-chatbot-top-messages":
            # Top messages table at bottom
            panels.append({
                "id": viz_id,
                "type": "visualization",
                "panelIndex": str(i+1),
    ...
    dashboard_data = {
        "attributes": {
            ...
            "hits": 0,
            "description": "Monitoring dashboard for Healthcare Chatbot logs",
